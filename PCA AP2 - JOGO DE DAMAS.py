...
class Jogo:
    ...
    def desenha(self):
        ...
        for l in range(len(matriz)):
            ...
            for c in range(len(matriz[l])):
                if matriz[l][c] == '#':
                    ...
                    pygame.draw.rect(display, PRETO, (x, y, 75, 75))
                x += 75
            y += 75
        ...
        if self.cedula_selecionada:
            ...
            if obrigatorios != {}:
                if (self.cedula_selecionada[0], self.cedula_selecionada[1]) not in obrigatorios:
                    ...
                    pygame.draw.rect(display, VERMELHO_2, (x_vermelho, y_vermelho, 75, 75))
                else:
                    if movs[0] == []:
                        x_vermelho = ALTURA / 8 * self.cedula_selecionada[1]
                        y_vermelho = ALTURA / 8 * self.cedula_selecionada[0]
                        ...
                        for i in range(len(movs[0])):
                            ...
                            pygame.draw.rect(display, VERMELHO_2, (x_possivel, y_possivel, 75, 75))
            else:
                if self.pulando:
                    x_vermelho = ALTURA / 8 * self.cedula_selecionada[1]
                    y_vermelho = ALTURA / 8 * self.cedula_selecionada[0]
                ...
                if elemento != '-':
                    ...
                    if elemento.lower() == 'x':
                        ...
                        if elemento == 'X':
                            ...
                            pygame.draw.circle(display, PRETO, (x, y), 5, 0)
                    else:
        ...
        if self.status != 'FIM DE JOGO':

            surface_texto, rect_texto = text_objects("VERMELHO: " + str(12 - o), fonte, VERMELHO_2)
            rect_texto.center = (650, 30)
            display.blit(surface_texto, rect_texto)

            surface_texto, rect_texto = text_objects("PRETO: " + str(12 - x), fonte, PRETO)
            rect_texto.center = (650, ALTURA - 30)
            display.blit(surface_texto, rect_texto)

            if self.turno % 2 == 1:
                surface_texto, rect_texto = text_objects("Turno do PRETO", fonte, PRETO)
                rect_texto.center = (700, ALTURA / 2)
                display.blit(surface_texto, rect_texto)
            else:
                surface_texto, rect_texto = text_objects("Turno do VERMELHO", fonte, VERMELHO_2)
                rect_texto.center = (700, ALTURA / 2)
                display.blit(surface_texto, rect_texto)
        else:
            surface_texto, rect_texto = text_objects("Game Over", fonte, PRETO)
...
def cria_botao(msg, sqr, cor1, cor2, cor_texto, acao=None):
    ...
    fontePequena = pygame.font.SysFont('Sans Serif', 30)
...
def creditos():
    ...
    while not sair:
        ...
        display.fill(BRANCO)
        fonte = pygame.font.SysFont('Sans Serif', 30)
        surface_texto, rect_texto = text_objects("VINICIUS RESENDE", fonte, PRETO)
        rect_texto.center = ((LARGURA / 2), ALTURA / 5)
        display.blit(surface_texto, rect_texto)

        surface_texto, rect_texto = text_objects("MATRÍCULA: 5406124", fonte, PRETO)
        rect_texto.center = ((LARGURA / 2), ALTURA / 4)
        display.blit(surface_texto, rect_texto)

        surface_texto, rect_texto = text_objects("PCA: INTRODUÇÃO AO DESENVOLVIMENTO DE SISTEMAS", fonte, VERMELHO_2)
        rect_texto.center = ((LARGURA / 2), ALTURA / 2.7)
        display.blit(surface_texto, rect_texto)

        voltar = fonte.render('Pressione qualquer tecla para voltar ao menu. LM', False, VERMELHO_2)
...
def regras():
    ...
    while not sair:
        ...
        fonte = pygame.font.SysFont('Sans Serif', 25)

        info1 = fonte.render('Jogo de damas ou simplesmente damas é um jogo de tabuleiro,', False, (PRETO))
        info2 = fonte.render('O objetivo do jogo é capturar todas as peças do oponente.', False, (VERMELHO))
        info3 = fonte.render('A peça anda só anda para frente, uma casa de cada vez, na diagonal.', False, (VERMELHO))
        info4 = fonte.render('Quando a peca atinge a oitava linha do tabuleiro ela vira dama.', False, (VERMELHO))
        info5 = fonte.render('A dama é uma peça de movimentos mais amplos. Ela anda para frente e para trás', False, (PRETO))
        info6 = fonte.render(' quantas casas quiser, não podendo saltar sobre uma peça da mesma cor.', False, (PRETO))
        info7 = fonte.render('A captura e obrigatória, ou seja, nao existe sopro.', False, (VERMELHO))
        info8 = fonte.render('Duas ou mais pecas juntas, na mesma diagonal, não podem ser capturadas.', False,(VERMELHO))
        info9 = fonte.render('A peca e a dama podem capturar tanto para frente como para tras.', False, (PRETO))
        info10 = fonte.render('O movimento de captura pode ser encadeado sem que o jogador passe a vez.', False, (PRETO))
        ...
        game3 = fonte.render('ela nao tem movimentos possiveis ou o turno pertence ao outro jogador.', False,(PRETO))

        voltar = fonte.render('Pressione qualquer tecla para voltar ao menu.', False, VERMELHO_2)
...
def tela_vencedor(vencedor):
    ...
    while not sair:
        ...
        fonte = pygame.font.SysFont('Sans Serif', 50)

        surface_texto, rect_texto = None, None

        if vencedor == "EMPATE":
            surface_texto, rect_texto = text_objects("EMPATE!", fonte, PRETO)
        elif vencedor == "x":
            surface_texto, rect_texto = text_objects("VITÓRIA DO  VERMELHO", fonte, VERMELHO)
        elif vencedor == "o":
            surface_texto, rect_texto = text_objects("VITÓRIA DO PRETO", fonte, PRETO)
...
def menu_jogo():
    while True:
        ...
        display.fill(BRANCO)
        fonte = pygame.font.SysFont('Sans Serif', 80)
        surface_texto, rect_texto = text_objects("Jogo de Damas", fonte, PRETO)
        rect_texto.center = ((LARGURA / 2), ALTURA / 3)
        display.blit(surface_texto, rect_texto)

        cria_botao("INICIAR", (LARGURA - 760, ALTURA / 2, 120, 40), VERMELHO_2, VERMELHO, PRETO, loop_jogo)
        cria_botao("MANUAL", (LARGURA - 560, ALTURA / 2, 120, 40), PRETO, VERMELHO, BRANCO, regras)
        cria_botao("CRÉDITOS", (LARGURA - 360, ALTURA / 2, 120, 40), PRETO, VERMELHO, BRANCO, creditos)
        cria_botao("SAIR", (LARGURA - 160, ALTURA / 2, 120, 40), VERMELHO_2, VERMELHO, PRETO, sair)
